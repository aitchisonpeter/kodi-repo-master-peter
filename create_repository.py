...
def addon_exists(root, addon_metadata):
    return root.findall(
        "./addon[@id='{id}'][@version='{version}']".format(
            id=addon_metadata.root.get('id'),
            version=addon_metadata.root.get('version')))
...
def create_repository(
    ...
    # if is_compressed:
    #     info_file = gzip.open(info_path, 'rb')
    # else:
    #     info_file = open(info_path, 'rb')
    # with info_file:
    #     tree = xml.etree.ElementTree.parse(info_file)
    tree = xml.etree.ElementTree.ElementTree(xml.etree.ElementTree.Element('addons'))
    root = tree.getroot()
    ...
    for worker in workers:
        ...
        if not addon_exists(root, result.addon_metadata):
